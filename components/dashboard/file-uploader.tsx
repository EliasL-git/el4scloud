  ...
  const uploadAll = async () => {
    ...
    for (const pf of toUpload) {
      ...
      try {
          ...
          xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) resolve()
              ...
              try {
                ...
                if (body.warned) warned = true
                if (body.suspended) suspended = true
              } catch {}
              if (warned || suspended) {
                reject({ violation: true, fileName: pf.file.name, reason: errMsg, suspended })
              } else {
        ...
        // Check if this is a violation warning (needs user acknowledgment)
        if (err?.violation) {
          setViolation({ fileName: err.fileName, reason: err.reason, suspended: err.suspended })
          // Remove the offending file from pending list
          setPending((prev) => prev.filter((f) => f.id !== pf.id))
          return
        }
        ...
        toast.error(err.message || `Failed to upload ${pf.file.name}`)
      }
    }
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              PDFs, ZIPs, images, documents — max 500 MB
            </p>
          </div>
        </div>
