# el4scloud Terms of Service

**Last Updated:** June 21, 2026

These Terms of Service ("Terms") govern your use of el4scloud ("Service"), operated by Elias Lindholm ("Operator", "we", "us", or "our").

By accessing, creating an account, or using el4scloud, you agree to these Terms. If you do not agree with these Terms, you must not use the Service.

---

# 1. Service Description

el4scloud is a free cloud storage and file hosting service.

The Service allows users to upload, store, manage, and access files.

All data is stored within the European Union (EU).

The Service is provided free of charge unless otherwise stated.

---

# 2. Account Authentication

el4scloud uses Hack Club Auth for authentication.

By logging in through Hack Club Auth, you acknowledge that information required for authentication may be processed by Hack Club.

Third-party services operate under their own terms and privacy policies.

You are responsible for protecting your account credentials and all actions performed through your account.

---

# 3. User Content and Ownership

You retain ownership of all files and content you upload.

By using the Service, you confirm that all uploaded content:

* Is owned by you; or
* Is content you have permission, rights, or licenses to store and use.

You are solely responsible for the content stored under your account.

el4scloud does not claim ownership of user content.

You grant el4scloud permission to store, process, transmit, and manage your content only as necessary to provide and operate the Service.

---

# 4. Copyright and Intellectual Property

You may not upload, store, or distribute content that violates another person's:

* Copyright;
* Trademark rights;
* Intellectual property rights;
* Legal rights.

If copyrighted material is reported or discovered, the Operator may remove the content and take appropriate action.

Repeated violations may result in account suspension or termination.

---

# 5. Prohibited Content

You may not use el4scloud to store, upload, distribute, or share:

* Illegal content;
* Stolen data;
* Malware, viruses, ransomware, or malicious software;
* Content that violates applicable laws;
* Copyright-infringing content;
* Pornographic content;
* Sexually explicit material;
* Adult or NSFW content;
* Content intended to harm, exploit, threaten, or abuse others.

NSFW content is not permitted and may be removed without notice.

---

# 6. Acceptable Use

You may not:

* Attempt unauthorized access to the Service;
* Attack or disrupt the Service;
* Abuse system resources;
* Circumvent security protections;
* Use the Service for fraud, phishing, spam, or malicious activity;
* Interfere with other users.

---

# 7. Commercial Use and Resale

el4scloud is provided free of charge for personal, educational, and authorized use.

Without written permission from the Operator, you may not:

* Resell access to el4scloud;
* Charge money for access to the Service;
* Include el4scloud in a paid product or service;
* Redistribute, sublicense, rent, or lease the Service;
* Provide paid hosting services using el4scloud.

You may not claim el4scloud as your own service.

Violations may result in immediate suspension or termination.

---

# 8. Law Enforcement Cooperation

The Operator may cooperate with law enforcement authorities when required by law or when there is reasonable evidence of illegal activity.

Illegal content or activity may be reported to relevant authorities, including the Swedish Police Authority.

Information that may be provided where legally permitted or required may include:

* IP addresses;
* Account information;
* Login records;
* Metadata;
* File information;
* Timestamps;
* Usage information.

Such information will only be shared when legally justified.

---

# 9. User Responsibility and Legal Liability

Users are solely responsible for the content they store on el4scloud.

The Operator is not responsible for:

* The legality of user files;
* Ownership disputes;
* User-uploaded content;
* Actions performed using user accounts.

If your use of el4scloud causes legal claims, investigations, damages, fines, or other costs, you are responsible for those consequences.

This includes:

* Copyright claims;
* Illegal content investigations;
* Unauthorized data sharing;
* Violations of laws or regulations.

You agree to indemnify and hold harmless the Operator from claims, damages, losses, and legal expenses caused by your use of the Service or stored content, to the maximum extent permitted by law.

---

# 10. Data Storage and Privacy

All data is stored within the European Union.

Reasonable technical and organizational measures may be used to protect user data.

However, no online service can guarantee complete security.

Users acknowledge that internet-based services carry risks.

---

# 11. Security and Data Loss Disclaimer

The Operator is not responsible for guaranteeing:

* Permanent file availability;
* Complete security;
* Prevention of all breaches;
* Prevention of all data loss.

The Operator is not responsible for damages caused by:

* Data loss;
* Data corruption;
* Unauthorized access;
* Security breaches;
* Service outages;
* Hardware failures;
* Technical issues.

Users are responsible for maintaining backups of important data.

You may not seek compensation from the Operator for data loss, downtime, or security incidents except where required by applicable law.

---

# 12. Third-Party Services

el4scloud may use third-party services including:

* Authentication providers;
* Hosting providers;
* Infrastructure providers;
* Security services.

Third parties may process information necessary to provide their services.

The Operator is not responsible for third-party services, availability, security, or policies.

---

# 13. Free Service and Availability

el4scloud is a free service.

No uptime guarantee, SLA, or availability commitment is provided.

Users acknowledge that:

* Uptime may be below 80%;
* Outages may occur;
* Maintenance may occur without notice;
* Features may change;
* The Service may be discontinued.

el4scloud should not be used for mission-critical, business-critical, emergency, or irreplaceable data.

---

# 14. Backups

Backups may exist but are not guaranteed.

Users must maintain their own copies of important files.

The Operator is not responsible for missing backups or lost data.

---

# 15. Content Removal and Account Termination

The Operator may remove content or terminate accounts that:

* Violate these Terms;
* Break applicable laws;
* Threaten security;
* Harm the Service or other users.

Actions may be taken without prior notice.

---

# 16. Disclaimer of Warranty

The Service is provided:

"AS IS" and "AS AVAILABLE."

The Operator does not guarantee:

* Reliability;
* Security;
* Availability;
* Accuracy;
* Error-free operation.

---

# 17. Limitation of Liability

To the maximum extent permitted by law, the Operator is not liable for:

* Data loss;
* Service interruptions;
* Security incidents;
* Lost profits;
* Business losses;
* Indirect or consequential damages.

If liability cannot legally be excluded, the Operator's maximum liability shall be limited to the maximum amount permitted by law.

---

# 18. Changes to the Service

The Operator may:

* Modify features;
* Change functionality;
* Suspend services;
* Stop operating el4scloud.

No compensation is provided for service changes or discontinuation.

---

# 19. Changes to These Terms

These Terms may be updated at any time.

Continued use of el4scloud after updates means you accept the updated Terms.

---

# 20. Governing Law

These Terms are governed by the laws of Sweden and applicable European Union law.

Any disputes shall be handled under Swedish jurisdiction.

---

# 21. Support

Support requests must be submitted through the el4scloud dashboard ticket system.

The Operator aims to respond within 48 hours, but response times are not guaranteed.

---

# 22. Contact

**Operator:** Elias Lindholm

For support requests, abuse reports, legal notices, or questions, use the el4scloud dashboard ticket system.

---

By using el4scloud, you confirm that you have read, understood, and agreed to these Terms of Service.


*edited on: 2026-06-21*